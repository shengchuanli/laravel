...
<body>
<center><h3>1908A商品修改</h3></center>
<form class="form-horizontal" action="{{url('/1908A/goods/update/'.$goodsfind->goods_id)}}" method="post" role="form" enctype="multipart/form-data">
    @csrf
    <div class="form-group">
        <label for="firstname" class="col-sm-2 control-label">商品名称</label>
        <div class="col-sm-3">
            <input type="text" name="goods_name"  value="{{$goodsfind->goods_name}}" class="form-control" id="firstname"
                   placeholder="请输入名称">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-3">
            <input type="text" name="goods_price" value="{{substr($goodsfind->goods_price,0,'-3')}}" class="form-control" id="firstname"
                   placeholder="请输入价格">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-3">
            ...
            <img src="{{env('UPLOAD_URL')}}{{$goodsfind->goods_img}}" height="50px" width="50px">
        </div>
    </div>
    <div class="form-group">
        <label for="firstname" class="col-sm-2 control-label">商品库存</label>
        <div class="col-sm-3">
            <input type="text" name="goods_num" value="{{$goodsfind->goods_num}}" class="form-control" id="firstname"
                   placeholder="请输入库存">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-3">
            <input type="radio" name="is_hot"  value="1" @if($goodsfind->is_hot==1) checked @endif id="firstname">是
            <input type="radio" name="is_hot"  value="2" @if($goodsfind->is_hot==2) checked @endif id="firstname">否
        </div>
    </div>
    <div class="form-group">
        <label for="firstname" class="col-sm-2 control-label">是否新品</label>
        <div class="col-sm-3">
            <input type="radio" name="is_new"  value="1" @if($goodsfind->is_new==1) checked @endif id="firstname">是
            <input type="radio" name="is_new"  value="2" @if($goodsfind->is_new==2) checked @endif id="firstname">否
        </div>
    </div>
    <div class="form-group">
        <label for="firstname" class="col-sm-2 control-label">是否首页展示</label>
        <div class="col-sm-3">
            <input type="radio" name="is_show"  value="1" @if($goodsfind->is_show==1) checked @endif id="firstname">是
            <input type="radio" name="is_show"  value="2" @if($goodsfind->is_show==2) checked @endif  id="firstname">否
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-sm-2">
            <select name="cate_id" id="" class="form-control">
                ...
                <option value="{{$v['cate_id']}}" @if($goodsfind->cate_id==$v['cate_id']) selected @endif>{!! str_repeat('&nbsp;&nbsp;',$v['level']*3)!!}{{$v['cate_name']}}</option>
                    @endforeach
            </select>
        ...
        <div class="col-sm-2">
            <select name="brand_id" id="" class="form-control">
                ...
                @foreach($brandall as $k=>$v)
                    <option value="{{$v->brand_id}}" @if($goodsfind->brand_id==$v->brand_id) selected @endif>
                        {{$v->brand_name}}</option>
                @endforeach
            </select>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-10">
            <textarea name="goods_desc" id="" cols="20" rows="6">{{$goodsfind->goods_desc}}</textarea>
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-sm-10">
            ...
            @if($goodsfind->goods_imgs)
                @php   $goods_imgs=explode('|',$goodsfind->goods_imgs);   @endphp
...
<script>
    ...
    $('input[type=button]').click(function(){
        ...
        if(!reg.test(goods_name)){
         ...
         var res=ajaxtest($('input[name=goods_name]'),goods_name,id);
            if (res===1){
...
//
    ...
    $('input[name=goods_name]').blur(function(){
        ...
        var goods_name= _this.val();
         ...
         var res=ajaxtest(_this,goods_name,id);
           if (res===1){
